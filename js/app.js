...
const abrirMenu = () => {
    ...
    // Agregar desplazamiento suave y cerrar menú
    const links = navegacion.querySelectorAll('a');
    links.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault(); // Prevenir el comportamiento por defecto

            const targetId = link.getAttribute('href'); // Ej: #servicios
            ...
            document.querySelector('.pantalla-completa')?.remove();
            document.querySelector('.btn-cerrar')?.remove();
        });
    });
};
